# Welcome, Traveler!

Welcome to my GitHub page — an humble ledger of quests, craft, and curiosities.

## About me

I am a student at 42 Heilbronn — a curious maker, tinkerer, and codewright who walks the crossroads between art and engineering. Here you'll find my school projects, current quests, and offerings of knowledge. Take a torch and look around; if something pleases thee, leave a mark.

---

## Current Quests

- Seeking mastery of `C++`, `C`, and sundry system-lore.
- Building tiny tools and tinkering with embedded and server-side spells.
- Learning new incantations in webcraft and container-sorcery.

## Skills & Crafts

- Languages: C, C++, Bash, a smattering of Python and JavaScript
- Tools: Git, Make, Docker
- Specialties: low-level tinkering, algorithms, systems, and clean, pragmatic design

## Notable Works (highlights)

- [cub3d](https://github.com/matthiaslendle/cub3d) — a small 3D raycasting demo; graphics and math.
- [ft_irc](https://github.com/matthiaslendle/ft_irc) — a school-built IRC server; networking and concurrency.
- [Inception](https://github.com/matthiaslendle/Inception) — containerization and deployment trials.
- ft_transcendence — my most valuable quest yet: a web-based game built with SvelteKit and Typescript.

Many more quests and exercises live inside the repository; explore the folders to read the code and run the demos.

## How to Summon Me

- Raven (email): leave a message in the realm of `mlendle@student.42heilbronn.de`.
- Carrier-pigeon (GitHub): open an Issue or a Pull Request on any repository herein.

## A Few Gentle Rules

- Be curious and kind.
- Share knowledge freely; code as if others will stand upon thy shoulders.
- Respect the craft and the craftsperson.

---

If thou wouldst like this hall to bear thy name and arms, tell me thy sigil and I shall scribe it.

May your compiles be swift and your tests green.

— A humble codewright
